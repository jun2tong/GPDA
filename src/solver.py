...
class Solver(object):
    ...
    def train(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            with torch.no_grad():
                phig_t = self.phig(img_s)
                f_t = torch.mm(phig_t, self.qw.mu)
            ...
            if batch_idx % self.interval == 0:
                ...
                print(prn_str)
                print(f"train acc on S: {100 * correct / n_sample: .4f}%")
                if record_file:
                    record = open(record_file, 'a')
                    record.write('%s\n' % (prn_str,))
                    record.write(f"train acc on S: {100 * correct / n_sample: .4f}%\n")
                    record.close()
    ...
    def test(self, epoch, record_file=None, save_model=False):
        ...
        prn_str = f'Test set: Average nll loss: {test_loss: .4f}, ' \
                  f'Accuracy: {corrects}/{size} ({100. * corrects / size : .6f})\n'
        print(prn_str)
        s_prn_str = f"validation acc on S: {100 * s_corrects / size: .4f}%"
        print(s_prn_str)
